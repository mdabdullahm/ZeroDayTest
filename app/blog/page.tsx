...
import { 
  Calendar, Clock, ArrowRight, BookOpen, 
  Terminal, AlertCircle, Loader2, ChevronDown
} from 'lucide-react';
import Link from 'next/link';
...
export default function BlogListPage() {
  const [blogs, setBlogs] = useState<IByteCapsuleBlog[]>([]);
  const [loading, setLoading] = useState(true);
  const [fetchingMore, setFetchingMore] = useState(false); // See More এর জন্য লোডিং
  const [error, setError] = useState<string | null>(null);
  const [page, setPage] = useState(1); // বর্তমান পেজ নম্বর
  const [hasMore, setHasMore] = useState(true); // আরও ব্লগ আছে কি না চেক করার জন্য

  const LIMIT = 9;

  // ব্লগ ফেচ করার মেইন ফাংশন
  const fetchBlogs = async (pageNum: number, isInitial: boolean = false) => {
    try {
      if (isInitial) setLoading(true);
      else setFetchingMore(true);

      const res = await fetch(`${BASE_URL}/api/public/blogs?page=${pageNum}&limit=${LIMIT}`, {
        cache: 'no-store'
      });
      ...
      if (isInitial) {
        ...
        // আগের ব্লগের সাথে নতুনগুলো যোগ করা
        setBlogs(prev => [...prev, ...newPosts]);
      }

      // যদি ৯টার কম পোস্ট আসে, তার মানে আর কোন পোস্ট নেই
  ...
  if (error) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-6 text-center">
      <div className="space-y-4 p-10 border border-red-500/20 bg-red-500/5 rounded-[3rem] max-w-lg">
        <AlertCircle className="text-red-500 mx-auto" size={48} />
        <h2 className="text-white font-bold uppercase">System_Link_Failure</h2>
        <p className="text-red-400 font-mono text-[10px]">{error}</p>
        <button onClick={() => window.location.reload()} className="px-8 py-3 bg-red-500 text-black font-black rounded-full text-[10px] uppercase">Re-establish_Link</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-black text-gray-300 font-sans selection:bg-green-500 selection:text-black pt-32 pb-20 overflow-x-hidden">
      <div className="max-w-full mx-auto px-6 lg:px-20 relative z-10">
        ...
        <div className="mb-20 space-y-4">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-6xl md:text-8xl font-black text-white uppercase tracking-tighter italic leading-[0.9]">
                OUR <span className="text-green-500">PROTOCOL</span> LOGS
            </motion.h1>
        </div>
          ...
          {loading ? (
            ...
            blogs.map((post, index) => (
              <motion.article 
                ...
                className="group bg-zinc-950/50 border border-white/5 rounded-[3rem] overflow-hidden hover:border-green-500/40 transition-all duration-500"
              >
                <div className="aspect-[16/11] overflow-hidden relative">
                   {post.coverImage ? (
                     <img src={post.coverImage} alt={post.title} className="w-full h-full object-cover opacity-70 group-hover:opacity-100 transition-transform duration-1000 group-hover:scale-110" />
                   ) : (
                     <div className="w-full h-full bg-zinc-900 flex items-center justify-center"><BookOpen size={48} className="text-zinc-800" /></div>
                   )}
                   <div className="absolute top-6 left-6 px-4 py-1.5 bg-black/80 backdrop-blur-xl border border-white/10 rounded-full flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div>
                ...
                <div className="p-10 space-y-5">
                  ...
                  <h3 className="text-2xl font-bold text-white uppercase tracking-tighter italic group-hover:text-green-400 transition-colors">{post.title}</h3>
                  <p className="text-gray-500 text-[11px] leading-relaxed line-clamp-3 font-mono uppercase">{post.excerpt || "Access full documentation for details."}</p>
                  <div className="pt-4">
        ...
        {hasMore && !loading && (
          <div className="mt-20 flex justify-center">
            <button 
              ...
              className="group relative px-12 py-4 bg-transparent border border-green-500/30 rounded-full overflow-hidden hover:border-green-500 transition-all duration-500 disabled:opacity-50"
            >
              <div className="relative z-10 flex items-center gap-3 text-green-500 font-mono text-[11px] font-black tracking-[0.3em] uppercase">
                {fetchingMore ? (
                  <>
        ...
        {!hasMore && blogs.length > 0 && (
          <div className="mt-20 text-center text-gray-600 font-mono text-[9px] uppercase tracking-[0.5em]">
             --- ALL_PROTOCOLS_DECRYPTED ---
          </div>
        )}
      ...
      <div className="max-w-full mx-auto px-6 lg:px-20 mt-20">
         <div className="h-px w-full bg-gradient-to-r from-transparent via-white/10 to-transparent"></div>
         <div className="py-8 flex justify-between items-center text-[9px] font-mono text-gray-600 uppercase tracking-widest">
            <span>Terminal: BC-INTEL-FEED-v1.0</span>
            <span className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${fetchingMore ? 'bg-yellow-500 animate-pulse' : 'bg-green-500'}`}></span> 
              {fetchingMore ? 'SYNCING_DATA' : 'System_Online'}
            </span>
         </div>
      </div>
    </div>
